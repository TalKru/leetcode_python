...
class Solution:
    def removeElement(self, nums: list[int], val: int) -> int:
        ...
        # for each element, if it isn't val, write it at nums[i] and bump write
        for i in range(len(nums)):
            if nums[i] != val:
                nums[write] = nums[i]
                write += 1

        return write  # 'write' is exactly the count of non-val elements
...
if __name__ == "__main__":
    ...
    obj1 = Solution()
    index = obj1.removeElement(nums, 2)
